...
# Pre-load ONNX sessions globally in main.py as requested
YOLO8M_SESSION = None
...
try:
    ...
    logger.warning(f"Could not initialize global ONNX sessions: {e}")
    ...
    if not skip_models:
        ...
        try:
            ...
            app.state.detection_service = DetectionService(
                ...
                yolo_session=YOLO8M_SESSION, 
                helmet_session=YOLO26N_SESSION
            )
            logger.info("DetectionService loaded from %s", settings.MODEL_DIR)
        except SystemExit:
            raise
